...
| 中文 | CER | 沒有空格，無詞界線 |
| 日文 | CER | 沒有空格，無詞界線 |
| 韓文 | CER | 雖有空格但不穩定，黏著語構造易誤判 |

- 黏著語特性 (agglutinative language)
  - 韓文詞彙常由詞幹 + 助詞 + 語尾組成，例如：

    ```
    먹었다 → 吃了（動詞詞幹 + 過去時 + 終結語尾）
    ```
    - 這使得同一個語意，可能因為斷詞方式不同而被當作不同的詞，WER 就會因此失真。

另外需要注意的是，在我們前面的例子中，只是用了一句句子來示範如何計算 WER。
但在實際評估一套語音系統時，我們通常會使用數千筆句子組成的測試集，來得到更準確的結果。

在這樣的評估過程中，我們會把所有句子的 S（替換）、I（插入）、D（刪除）和 N（參考詞數）加總起來，然後再套用 WER 的公式來計算整體錯誤率。
這樣的做法能更可靠地評估模型在未知資料上的真實表現。
